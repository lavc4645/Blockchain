...
import "./PlayerNew.sol";
 ...
 contract GameNew{
     ...
     event PlayerJoined(uint index,address player);
    ...
    function startGame() onlyApproved() public {
     ...
     function setPlayerAccount(address payable player_) public returns(bool){
       // require(_state == GameState.Pause || _state == GameState.Start, "Game is not in running condition");
        PlayerNew _player = new PlayerNew();
        player.setNewPlayer(this, player, playerIndex);
    ...
    function getPlayerById(uint index) public view returns (Player memory) {
